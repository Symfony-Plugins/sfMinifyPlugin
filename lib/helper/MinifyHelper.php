...
function minify_get_javascripts()
{
  ...
  foreach($minify_files as $options => $files)
  {
    ...
    $options['src'] = '/minify/'.sfConfig::get('app_sfMinifyPlugin_asset_version', 1).join($files, ',');
...
function minify_get_stylesheets()
{
  ...
  {
    $options = unserialize($options);
    $options['href'] = '/minify/'.sfConfig::get('app_sfMinifyPlugin_asset_version', 1).join($files, ',');
